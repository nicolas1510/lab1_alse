...
int main()
{
        ...
        int opcion, datos[VALOR_MAX], datos1[VALOR_MAX];           //variables usadas atraves de la
        float desviacion, promedio;                                // rutina.
        char valore[VALOR_MAX];
        float  numeros_datos1, cantidad;        


// se realiza el menu del programa usando switch case, donde se obtendra todos las operaciones propuestas
...
// aquí recibiremos los primeros datos del usuario como una cadena de caracteres.
...
void ingreso_datos(char valores1[VALOR_MAX]) 
{
        ...
        printf("por favor ingrese los valores separados por ';'como se muestra:45;32;12;... \n");
        scanf("%s",valores1);

}

//se recibe el 2 dato dado por el usuario
...
//se crea la subrutina conversion con el fin de cambiar el valor del usuario de cadena a entero , y agregarlo en funcon de entero 
  ...
  for(contador=0;contador<=strlen(valores1)+1;contador++) 
        {
                ...
                {
                        datos[posicion]=atoi(destino);         //cambio de cadena a entero
                        posicion=posicion+1;               // cantidad de datos dados por el usuario
                        ...
                        for (contador1=1; contador1<VALOR_MAX; contador1++)
			{
                ...
                destino[contador1]=valores1[contador];  // se pasan los caracteres al vector destino
                contador1=contador1+1;
        }
 numero_dat=posicion;                           // si coloca un limite para la cantidad de datos 
 if (numero_dat<=100)
  {
        *cantidad=numero_dat;
  }
...
// se realiza lo mismo que en conversion ,este destinado para el 2 dato dado por el usuario
...
// se realiza el promedio mediante el vector datos ,el cual contiene los datos ingresados por el usuario en entero
 ...
 void promedio_val(float* promedio,float* cantidad, int datos[VALOR_MAX])
{
        ...
        for (contador=0 ; contador<numero; contador++)       //se realiza el recorrido
	{
             valor=valor+datos[contador];                  // se suman las variables dadas
        }
        valor=valor/numero;                                // se realiza el promedio
        printf("El promedio es: %5.3f \n",valor);   
        *promedio=valor;                            //de guarda en  promedio
...
//se realiza la desviacion del 1 dato dado por el usuario 
...
char maximo (int datos[VALOR_MAX], float cantidad)
{
  ...
  int max, medio=VALOR_MAX/2;
         ...
         for (cont=0;cont<medio;cont ++)             // se realiza un recorrido por el vector dato
        {
...
// se realiza la comparacion entre las dos cadenas , caracter por caracter.
...
//se compara la suma  total de las cadenas de carácteres, para obtener la informacion de quien es el mas grande
